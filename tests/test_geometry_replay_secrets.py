"""Gann/pitchfork geometry, replay anti-lookahead, secret containment, provider status."""
...
from sam_backend.secrets import SUPPORTED_KEYS, SecretStore, resolve_credential
from sam_backend.trading.geometry import (
    GANN_ANGLES,
    PITCHFORK_VARIANTS,
    bar_minutes,
...
from sam_backend.trading.replay import BarReplayResearch, LookaheadError, ReplaySession, ReplayState
from sam_backend.trading.types import Candle

START = datetime(2026, 7, 1, 8, 0, tzinfo=UTC)
SENTINEL = "sk-or-v1-SECRETSTORE-TEST-SENTINEL-0123456789"
...
def test_native_tradingview_replay_is_reported_partial_not_verified():
    capability = BarReplayResearch(market_data=None).capability()
...
def test_controlling_a_missing_session_is_refused():
    research = BarReplayResearch(market_data=None)
    result = research.control("advance")
    assert result.error_code == "NO_REPLAY_SESSION"
...
def test_a_stored_key_is_never_returned_by_the_public_status(store: SecretStore):
    ...
    # The fingerprint identifies the key without revealing it.
    assert len(status["openrouter_api_key"]["fingerprint"]) == 12


def test_the_secret_file_is_not_inside_the_web_root(store: SecretStore, tmp_path: Path):
    store.set("openrouter_api_key", SENTINEL)
    assert store.path.is_file()
    assert "frontend" not in store.path.parts


def test_a_malformed_key_is_rejected_before_any_network_call(store: SecretStore):
    for invalid in ("", "   ", "not-a-key", "sk-or"):
        with pytest.raises(ValueError):
            store.set("openrouter_api_key", invalid)


def test_an_unsupported_credential_name_is_refused(store: SecretStore):
    with pytest.raises(ValueError, match="Unsupported credential"):
        store.set("aws_secret", SENTINEL)


def test_the_environment_takes_precedence_over_the_store(store: SecretStore, monkeypatch):
    store.set("openrouter_api_key", SENTINEL)
    monkeypatch.setenv("OPENROUTER_API_KEY", "sk-or-v1-FROM-ENVIRONMENT-9876543210")
    value, source = resolve_credential("openrouter_api_key", store)
    assert source == "environment"
    assert value != SENTINEL


def test_the_store_supplies_the_key_when_the_environment_is_empty(store: SecretStore, monkeypatch):
    monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
    store.set("openrouter_api_key", SENTINEL)
    value, source = resolve_credential("openrouter_api_key", store)
    assert source == "secret_store"
    assert value == SENTINEL


def test_an_unset_credential_reports_unset(store: SecretStore, monkeypatch):
    monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
    assert resolve_credential("openrouter_api_key", store) == (None, "unset")


def test_clearing_a_credential_removes_it(store: SecretStore):
    store.set("openrouter_api_key", SENTINEL)
    assert store.clear("openrouter_api_key") is True
    assert store.get("openrouter_api_key") is None
    assert store.clear("openrouter_api_key") is False


def test_every_supported_key_has_a_validation_pattern():
    from sam_backend.secrets import KEY_PATTERNS

    assert set(KEY_PATTERNS) == set(SUPPORTED_KEYS)


def test_the_fingerprint_is_stable_and_not_the_key():
    first = SecretStore.fingerprint(SENTINEL)
    assert first == SecretStore.fingerprint(SENTINEL)
    assert first not in SENTINEL and SENTINEL not in (first or "")
    assert SecretStore.fingerprint(None) is None
