"""Classic indicators over closed candles (ported unchanged from v1
``sam_backend/trading/indicators.py``; pure arithmetic, no I/O).

Every series function returns a list aligned with its input, ``None`` where
the indicator is not yet defined, so callers never shift indices by hand.
"""
