...
def snr_levels(candles: list[Candle], timeframe: str, max_levels: int = 16) -> list[PriceLevel]:
    ...
    return sorted(levels, key=lambda level: level.score, reverse=True)[:max_levels]
...
def entry_hunter(
    ...
    *,
    ...
    if direction == Direction.BULLISH:
        ...
        invalidation_candidates = _nearest_levels(analyses, price, "below")
        target_candidates = _nearest_levels(analyses, price, "above")
    else:
        ...
        invalidation_candidates = _nearest_levels(analyses, price, "above")
        target_candidates = _nearest_levels(analyses, price, "below")
    confirmations = {
    ...
    if not invalidation_candidates:
        return {
            ...
            "direction": direction.value,
            "confirmations": confirmations,
            "missing_confirmation": ["Technical invalidation level"],
            "reasons": ["No defensible structural invalidation level is available."],
        }
    invalidation = float(invalidation_candidates[0]["price"])
    atr_value = float(ltf["indicators"].get("atr14") or 0.0)
    ...
    stop = invalidation - buffer if direction == Direction.BULLISH else invalidation + buffer
    risk = abs(price - stop)
    if risk <= 0:
    ...
    target_prices = [float(item["price"]) for item in target_candidates if (float(item["price"]) - price) * (1 if direction == Direction.BULLISH else -1) > 0]
    target_prices = sorted(set(target_prices), reverse=direction == Direction.BEARISH)
    targets = []
    for target in target_prices[:3]:
        reward = abs(target - price)
        targets.append({"price": target, "rr": reward / risk, "technical_source": next((item.get("kind") for item in target_candidates if float(item["price"]) == target), "STRUCTURE")})
    if not targets:
        return {
            "decision": SetupDecision.WAIT.value,
            "state": SetupState.WAITING_FOR_TRIGGER.value,
            "direction": direction.value,
            "confirmations": confirmations,
            "missing_confirmation": ["Technical target"],
            "reasons": ["No technical liquidity or structure target is available; SAM will not invent a target."],
        }
